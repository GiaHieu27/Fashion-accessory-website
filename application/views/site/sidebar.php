...
<div class="col-xs-12 col-sm-3 col-md-3 col-lg-3 clearpaddingl">
	...
	<div class="panel panel-info">
		...
		<div class="panel-body" style="padding:0px">
			<form class="form-horizontal text-center" method="post" action="<?php echo base_url('product/search'); ?>">
				<div class="form-group form-group-sm">
					<label class="col-sm-5 clearpaddingl control-label" for="formGroupInputSmall">
						Danh mục
					</label>
					...
					<div class="col-sm-7" style="padding-left: 0px">
						<select class="form-control" name="catalog_id">
							<?php foreach ($catalog as $value) { ?>
								<option value="<?php echo $value->id; ?>" style='font-weight: bold' <?php if ($value->id == $catalog_id) echo 'selected'; ?>>
									<?php echo $value->name; ?>
								</option>
								<?php foreach ($value->sub as $val) { ?>
									<option value="<?php echo $val->id; ?>" <?php if ($val->id == $catalog_id) echo 'selected'; ?>>
										&nbsp;&nbsp;&nbsp;<?php echo $val->name; ?>
									</option>
								<?php } ?>
					</div>
		...
		<button class="btn__sidebar" type="submit" name='submit'>Tìm kiếm</button>
		</form>
	</div>
</div>
<div class="panel panel-info">
	<div class="panel-body" style="padding:0px">
		<div class="primary" style="margin-top: 38px;">
			<h3>Danh mục</h3>
		</div>
		<div class="list-group">
			...
			?>
				...
				<div class="list-group">
					...
					?>
						<a href="<?php echo base_url($namesub . '-c' . $val->id); ?>" class="list-group-item list__cus"><?php echo $val->name; ?></a>
					<?php } ?>
				</div>
			<?php } ?>
		</div>
